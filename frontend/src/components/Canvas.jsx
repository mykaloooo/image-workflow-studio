...
import { generateImage, uploadImage, createVideoTask, getVideoTaskStatus as pollVideoStatus } from '../utils/api'
...
import StoryboardCreateModal from './Storyboard/StoryboardCreateModal'
import { LAYOUTS, computeSlotPositions, generateStoryboardId } from './Storyboard/storyboardLayouts'
import { getSlotLabel, getSlotPrompt } from './Storyboard/storyboardPrompts'
...
function Canvas({ apiConfig, onOpenSettings, onOpenAgentManager, onOpenTemplateManager }) {
  ...
  // 故事板创建弹窗 (Phase 2) - 受 feature flag 控制
  const storyboardEnabled = useFeatureFlag(FLAGS.STORYBOARD_ENABLED)
  const [storyboardModal, setStoryboardModal] = useState(null)
  ...
  const addLog = useCallback((message, level = 'info') => {
    ...
    if (safeMessage.includes('data:image')) {
      safeMessage = safeMessage.replace(/data:image\/[^,;\s]+;base64,[A-Za-z0-9+/=\r\n]+/g, (m) => `${m.slice(0, 64)}...[data-uri ${m.length} chars]`)
    }
    if (safeMessage.length > MAX_LOG_CHARS) {
      safeMessage = `${safeMessage.slice(0, MAX_LOG_CHARS)}...[truncated ${safeMessage.length} chars]`
    }
    setLogs(prev => {
      const newLogs = [...prev, { time, message: safeMessage, level }]
      // 超过最大条数时，删除旧日志
            ...
            n.id === nodeId
              ? {
                  ...
                  data: {
                    ...
                    // 生图历史记录 ID（画布下载时用来 PATCH canvas_save_state）
                    historyRecordId: result.history_record_id || null,
                    // 保留回调函数
        ...
        for (let i = 1; i < result.images.length; i++) {
          ...
          const additionalNode = {
            ...
            data: {
              ...
              // 保存参考图信息（用于生成记录显示）
              sourceReferenceImages: referenceImages.map(url => ({ url })),
              // 生图历史记录 ID（同一次调用共享，下载任一张都会 PATCH 同一条记录）
              historyRecordId: result.history_record_id || null,
              // 添加回调函数
  ...
  }, [addLog])  // 移除 nodes, edges 依赖，使用 ref 代替
  ...
  const createImportedImageNode = useCallback((imageInfo, position) => {
    ...
    const newNode = {
      ...
      data: {
        ...
        prompt: imageInfo.name ? `Eagle 导入: ${imageInfo.name}` : 'Eagle 导入图片',
        sequenceNum,
        onGenerate: (...args) => callbacksRef.current.handleGenerate(...args),
        onAddChild: (...args) => callbacksRef.current.handleAddChild(...args),
        onRegenerate: (...args) => callbacksRef.current.handleRegenerateNew(...args),
  ...
  useEffect(() => {
    ...
    const raw = params.get('importImages')
    if (!raw) return
    ...
    try {
      ...
      imageList.forEach((image, index) => {
        createImportedImageNode(image, {
          x: 120 + index * 340,
          y: 120 + (index % 2) * 40,
        })
      })
      if (imageList.length > 0) {
        addLog(`已从 Eagle 导入 ${imageList.length} 张图片`, 'success')
      }
      window.history.replaceState(null, '', window.location.pathname)
    } catch (error) {
      console.error('Eagle 导入失败:', error)
      addLog(`Eagle 导入失败: ${error.message}`, 'error')
    }
  }, [createImportedImageNode, addLog])
  ...
  // 故事板：右键菜单 → 打开弹窗 (Phase 2)
  const handleOpenStoryboardModal = useCallback(() => {
    if (!contextMenu) return
    const { x, y } = contextMenu
    const rect = canvasRef.current?.getBoundingClientRect()
    if (!rect) return
    const anchor = project({ x: x - rect.left, y: y - rect.top })
    setStoryboardModal({ anchor })
    setContextMenu(null)
  }, [contextMenu, project])

  // 故事板：批量创建空节点 (Phase 2)
  const handleCreateStoryboard = useCallback((payload) => {
    const { layoutId, providerId, model, aspectRatio, resolution, anchor } = payload
    const layout = LAYOUTS[layoutId]
    if (!layout) {
      addLog(`✗ 未知故事板布局: ${layoutId}`, 'error')
      setStoryboardModal(null)
      return
    }
    const storyboardId = generateStoryboardId()
    const positions = computeSlotPositions(layoutId, anchor)
    const newNodes = positions.map(({ slotIndex, position }) => {
      const slotKey = layout.slots[slotIndex] || `slot${slotIndex}`
      const slotLabel = getSlotLabel(slotKey)
      const promptTemplate = getSlotPrompt(slotKey)
      ...
      return {
        ...
        data: {
          ...
          model,
          sequenceNum,
          // 故事板分组元数据 (Phase 2-5 使用)
          storyboardId,
          slotIndex,
          slotKey,
          slotLabel,
          storyboardLayout: layoutId,
          storyboardProviderId: providerId,
          // 常规回调
          onGenerate: (...args) => callbacksRef.current.handleGenerate(...args),
          onAddChild: (...args) => callbacksRef.current.handleAddChild(...args),
          onRegenerate: (...args) => callbacksRef.current.handleRegenerateNew(...args),
    ...
    addLog(`✓ 已创建故事板组（${layout.name}），共 ${newNodes.length} 个节点`, 'success')
    setStoryboardModal(null)
  }, [addLog, setNodes])
  ...
  const handleRegenerateNew = useCallback(async (sourceNodeId, formData) => {
    ...
    try {
      ...
      console.log('[handleRegenerateNew] 参数:', {
        prompt: enhancedPrompt,
        aspect_ratio: formData.aspectRatio,
        resolution: formData.resolution,
        reference_images: referenceImages,
  ...
  // 回调函数 ref - 用于在节点中调用
  const callbacksRef = useRef({ handleGenerate, handleAddChild, handleChat, handleRegenerateNew, handleGenerateVideo, handleUpdateImage })
  ...
  const onNodesChangeWithCallbackRestore = useCallback((changes) => {
    ...
    setNodes(currentNodes => {
      const nodesNeedingCallbacks = currentNodes.filter(n =>
        ...
        (!n.data.onRegenerate || !n.data.onGenerate)
      )
  ...
  useEffect(() => {
    callbacksRef.current = { handleGenerate, handleAddChild, handleChat, handleRegenerateNew, handleGenerateVideo, handleUpdateImage }
  }, [handleGenerate, handleAddChild, handleChat, handleRegenerateNew, handleGenerateVideo, handleUpdateImage])
  ...
  const handleOpenProject = useCallback(async () => {
    try {
      ...
      const nodesWithHandle = project.nodes.map(n => ({
        ...n,
        dragHandle: n.type === 'imageNode' ? '.image-node-drag-handle' :
                   n.type === 'chatNode' ? '.chat-node-drag-handle' : undefined
      }))
      ...
      fileInput.onchange = async (e) => {
        ...
        try {
          ...
          if (nodes.length > 0) {
            ...
            if (merge === false) {
              const offset = nodeIdCounter.current
              const newNodes = project.nodes.map(node => {
                const newId = `node_${offset + 1}`
  ...
  return (
    <NodesProvider nodes={nodes} edges={edges}>
            ...
            <>
              ...
              {storyboardEnabled && (
                <>
                  ...
                  <div className="context-menu-item" onClick={handleOpenStoryboardModal}>
                    📑 创建故事板组...
                  </div>
                </>
              )}
      ...
      {/* 故事板创建弹窗 (Phase 2) - 受 feature flag 控制 */}
      {storyboardEnabled && storyboardModal && (
        <StoryboardCreateModal
          anchor={storyboardModal.anchor}
