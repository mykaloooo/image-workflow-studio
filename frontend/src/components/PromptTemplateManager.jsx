...
const templateTypeOptions = [
  { value: '', label: '未分类' },
  { value: 'quotation_exploded', label: '软装报价爆炸图' },
  { value: 'moodboard', label: '软装材料情绪板' },
  { value: 'analysis_board', label: '搭配分析图' },
  { value: 'reverse_prompt', label: '参考图反推提示词' },
  { value: 'image_enhance', label: '原图优化提示词' },
  { value: 'workflow', label: '工作流模板' }
]

const templateStatusOptions = [
  { value: '', label: '未标记' },
  { value: 'gold', label: '金样板' },
  { value: 'stable', label: '稳定' },
  { value: 'testing', label: '待测试' },
  { value: 'draft', label: '草稿' }
]

const defaultWorkflow = '视觉分析 JSON -> 提示词编译 -> 易狗/Eagle 生图 -> 结果回看'
...
function PromptTemplateManager({ onClose }) {
  ...
  const getSuptUrl = () => {
    return localStorage.getItem('iws_supt_url') || 'https://api.kakahome.top/supt/'
  }
  const setSuptUrl = () => {
    const cur = getSuptUrl()
    const v = prompt('SU Prompt Toolkit 地址（默认使用 115 服务器版）：', cur)
    if (v === null) return
    if (!v.trim()) {
  ...
  const getTemplateTypeLabel = (value) => templateTypeOptions.find(opt => opt.value === value)?.label || '未分类'
  const getTemplateStatusLabel = (value) => templateStatusOptions.find(opt => opt.value === value)?.label || '未标记'
  const getTemplateStatusColor = (value) => ({
    gold: '#D6A84F',
    stable: '#4CAF50',
    testing: '#FF9800',
    draft: '#777'
  ...
  return (
    <div className="config-panel-overlay" onClick={onClose}>
      <div className="config-panel" onClick={e => e.stopPropagation()} style={{ width: '90vw', maxWidth: '1200px', height: '85vh' }}>
        <div className="config-header">
          <h2>📝 提示词模板管理</h2>
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
            <button
              onClick={openSuptTool}
              style={{ background: '#4CAF50', border: 'none', borderRadius: '6px', padding: '7px 12px', color: 'white', cursor: 'pointer', fontSize: '13px' }}
              title="打开 SU Prompt Toolkit：反推、编译、保存模板"
            >
              🚀 打开提示词工厂
            </button>
            <button
              onClick={setSuptUrl}
              style={{ background: '#333', border: '1px solid #555', borderRadius: '6px', padding: '7px 10px', color: '#ddd', cursor: 'pointer', fontSize: '13px' }}
              title="配置提示词工厂地址"
            >
              ⚙ 工厂地址
            </button>
        ...
        <div style={{ display: 'flex', flex: 1, gap: '1px', background: '#333', overflow: 'hidden' }}>
          {/* 左侧：分组列表 */}
          <div style={{ width: '200px', background: '#1a1a1a', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ flex: 1, background: '#222', display: 'flex', flexDirection: 'column' }}>
            <div style={{ padding: '12px', borderBottom: '1px solid #333', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <button
                onClick={() => setEditingTemplate(createBlankTemplate())}
            ...
            {editingTemplate ? (
              <div style={{ flex: 1, padding: '16px', overflowY: 'auto' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '12px', marginBottom: '12px' }}>
                  <div>
                    <label style={{ color: '#999', fontSize: '12px', display: 'block', marginBottom: '4px' }}>模板类型</label>
                    <select
                      value={editingTemplate.templateType || ''}
                      onChange={e => setEditingTemplate({ ...editingTemplate, templateType: e.target.value })}
                      style={{ width: '100%', background: '#1a1a1a', border: '1px solid #444', borderRadius: '6px', padding: '10px', color: 'white', fontSize: '14px' }}
                    >
                      {templateTypeOptions.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
                    </select>
                  </div>
                  <div>
                    <label style={{ color: '#999', fontSize: '12px', display: 'block', marginBottom: '4px' }}>模板等级</label>
                    <select
                      value={editingTemplate.templateStatus || ''}
                      onChange={e => setEditingTemplate({ ...editingTemplate, templateStatus: e.target.value })}
                      style={{ width: '100%', background: '#1a1a1a', border: '1px solid #444', borderRadius: '6px', padding: '10px', color: 'white', fontSize: '14px' }}
                    >
                      {templateStatusOptions.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
                    </select>
                  </div>
                </div>
                <div style={{ marginBottom: '12px' }}>
                  <label style={{ color: '#999', fontSize: '12px', display: 'block', marginBottom: '4px' }}>三步流程</label>
                  <input
                    value={editingTemplate.workflow || ''}
                    onChange={e => setEditingTemplate({ ...editingTemplate, workflow: e.target.value })}
                    placeholder={defaultWorkflow}
                    style={{ width: '100%', background: '#1a1a1a', border: '1px solid #444', borderRadius: '6px', padding: '10px', color: 'white', fontSize: '14px' }}
                  />
                </div>
                <div style={{ marginBottom: '12px' }}>
                  <label style={{ color: '#999', fontSize: '12px', display: 'block', marginBottom: '4px' }}>视觉分析字段 / JSON骨架</label>
                  <textarea
                    value={editingTemplate.analysisSchema || ''}
                    onChange={e => setEditingTemplate({ ...editingTemplate, analysisSchema: e.target.value })}
                    placeholder="例如：空间类型、风格、主色、材质、家具、灯具、摆件、版式目标..."
                    rows={4}
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '12px', marginBottom: '12px' }}>
                  <div>
                    <label style={{ color: '#999', fontSize: '12px', display: 'block', marginBottom: '4px' }}>编译说明</label>
                    <textarea
                      value={editingTemplate.compileNotes || ''}
                      onChange={e => setEditingTemplate({ ...editingTemplate, compileNotes: e.target.value })}
                      placeholder="说明这个模板如何把分析结果填进提示词..."
                      rows={4}
                      style={{ width: '100%', background: '#1a1a1a', border: '1px solid #444', borderRadius: '6px', padding: '10px', color: 'white', fontSize: '14px', resize: 'vertical', fontFamily: 'inherit' }}
                    />
                  </div>
                  <div>
                    <label style={{ color: '#999', fontSize: '12px', display: 'block', marginBottom: '4px' }}>测试记录</label>
                    <textarea
                      value={editingTemplate.testNotes || ''}
                      onChange={e => setEditingTemplate({ ...editingTemplate, testNotes: e.target.value })}
                      placeholder="记录成功图、失败点、适合模型、下次调整..."
                      rows={4}
                      style={{ width: '100%', background: '#1a1a1a', border: '1px solid #444', borderRadius: '6px', padding: '10px', color: 'white', fontSize: '14px', resize: 'vertical', fontFamily: 'inherit' }}
                    />
                  </div>
                ...
                <div style={{ marginBottom: '12px' }}>
                  <label style={{ color: '#999', fontSize: '12px', display: 'block', marginBottom: '4px' }}>样板图路径 / URL</label>
                  <input
                    value={editingTemplate.sampleImage || ''}
                    onChange={e => setEditingTemplate({ ...editingTemplate, sampleImage: e.target.value })}
                    placeholder="例如：L:/TUKU upload/.../xxx.png"
                    style={{ width: '100%', background: '#1a1a1a', border: '1px solid #444', borderRadius: '6px', padding: '10px', color: 'white', fontSize: '14px' }}
                  />
                </div>
                ...
                {filteredTemplates.length === 0 ? (
                  ...
                  filteredTemplates.map(t => (
                    ...
                    >
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                        <div style={{ minWidth: 0 }}>
                          ...
                          <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginTop: '6px' }}>
                            <span style={{ fontSize: '11px', color: '#d6d6d6', background: '#444', borderRadius: '999px', padding: '2px 8px' }}>
                              {getTemplateTypeLabel(t.templateType)}
                            </span>
                            <span style={{ fontSize: '11px', color: '#111', background: getTemplateStatusColor(t.templateStatus), borderRadius: '999px', padding: '2px 8px' }}>
                              {getTemplateStatusLabel(t.templateStatus)}
                            </span>
                          </div>
                        </div>
                        <button
                      ...
                      {(t.workflow || t.testNotes) && (
                        <div style={{ marginTop: '8px', paddingTop: '8px', borderTop: '1px solid #3a3a3a', color: '#999', fontSize: '11px', lineHeight: '1.5' }}>
                          {t.workflow && <div>流程：{t.workflow}</div>}
                          {t.testNotes && <div>测试：{t.testNotes}</div>}
                        </div>
                      )}
                    </div>
                  ))
